...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    // Determine type
    const type = tags.abbr === 'Patent' ? 'patent' : entry.entryType.toLowerCase() === 'unpublished' ? 'manuscript' : typeMapping[entry.entryType.toLowerCase()] || 'journal';
    ...
    const publication: Publication = {
      ...
      type,
      status: /under review/i.test(tags.note || '') ? 'under-review' : /accepted/i.test(tags.note || '') ? 'accepted' : 'published',
      tags: keywords,
      keywords,
      researchArea: detectResearchArea(tags.title, keywords),

      // Optional fields
      journal: cleanBibTeXString(tags.journal || tags.howpublished),
      conference: cleanBibTeXString(tags.booktitle),
      volume: tags.volume,
      issue: tags.number,
      pages: tags.pages,
      doi: tags.doi,
      url: tags.url || tags.html,
      code: tags.code,
    ...
    // Show published research before manuscripts and patent applications.
    const order: Partial<Record<PublicationType, number>> = { journal: 0, conference: 1, manuscript: 2, patent: 3 };
    const typeOrder = (order[a.type] ?? 1) - (order[b.type] ?? 1);
    if (typeOrder) return typeOrder;
    // Sort each category by year (descending), then month.
...
function parseAuthors(authorsStr: string, highlightNames: string[]): Array<{ name: string; isHighlighted?: boolean; isCorresponding?: boolean; isCoAuthor?: boolean }> {
  ...
  return authorsStr
    ...
    .map(author => {
      ...
      name = name.toLowerCase() === 'others' ? 'et al.' : cleanBibTeXString(name);
