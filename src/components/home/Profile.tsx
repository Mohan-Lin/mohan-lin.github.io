...
// ResearchGate mark from Simple Icons (CC0); inherits the site icon color.
...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('mohan-lin-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('mohan-lin-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('mohan-lin-user-liked');
            setShowThanks(false);
        }
    };
    ...
    const socialLinks = [
        ...
        ...(social.researchgate ? [{ name: 'ResearchGate', href: String(social.researchgate), icon: ResearchGateIcon }] : []),
        ...(social.linkedin ? [{
            name: 'LinkedIn',
